...
- `PF-A.H2` Verify the phone OTP · F-101
- `PF-A.H3` Log in (phone + password, or phone + OTP) · F-102
- `PF-A.H4` Recover password · F-103
- `PF-A.H5` Verify identity / KYC · F-105
...
- `PF-B.H4` Upload lab results · F-113
- `PF-B.H5` Set delivery address · F-115
- `PF-B.H6` Answer eligibility questionnaire · F-121
- `PF-B.H7` Compute eligibility, show result · F-122, F-123, F-125, F-126
...
- `PF-C.H2` Purchase & pay · F-131
- `PF-C.H3` Manage / stop renewal · F-133
- `PF-C.H4` Cancel + exit interview · F-139
- `PF-C.H5` Wallet — balance, top-up, pay · F-141, F-142, F-143, F-146
...
- `PF-D.H3` Join the phone visit · F-153
- `PF-D.H4` View & download the prescription · F-155, F-159
...
**Happy**
- `PF-F.H1` Report a side effect · F-177, F-178
...
- `MF-A.H2` BMI calculator · F-096
- `MF-A.H3` Public eligibility check (preliminary) · F-097
...
- `DF-A.H2` Open a patient profile + EMR · F-312
- `DF-A.H3` Conduct the phone visit · F-153
- `DF-A.H4` Write the SOAP note · F-328
- `DF-A.H5` Link the external prescription · F-327, F-329
...
**Happy**
- `DF-B.H1` Doctor login · F-102
- `DF-B.H2` Doctor dashboard · F-301, F-305, F-302, F-303, F-304
- `DF-B.H3` Patient management · F-311, F-313, F-314, F-315, F-317, F-319, F-316
...
- `NF-A.H4` GPS check-in at the patient's location · F-430
- `NF-A.H5` View patient profile + today's dose · F-431, F-432
- `NF-A.H6` Record cold-chain temperature · F-433
- `NF-A.H7` Pre-injection assessment · F-434
- `NF-A.H8` Scan vial, then record injection site · F-437
- `NF-A.H9` Patient signs · F-441
...
- `NF-B.H2` Press the safety button · F-497
...
**Happy**
- `NF-C.H1` Live tracking + ETA · F-707, F-708, F-172
- `NF-C.H2` Assigned-patient list · F-450, F-451, F-452, F-453, F-454
- `NF-C.H3` Nurse inventory · F-460, F-465, F-461, F-462, F-463, F-464
...
- `NF-C.H9` Performance + hotline · F-485, F-486, F-495
...
- `PH-A.H2` Inventory dashboard · F-502
- `PH-A.H3` Record supplier receipt · F-503
- `PH-A.H4` Monitor cold chain · F-505
- `PH-A.H5` Dispense to nurse · F-504
- `PH-A.H6` Report damage · F-506
...
**Happy**
- `PH-B.H1` Recall a batch — cascade to its vials · F-505, F-618
...
**Happy**
- `OP-D.H1` Issue a refund · F-147
- `OP-D.H2` Run monthly settlements · F-627, F-628, F-629
- `OP-D.H3` Finance dashboard · F-625, F-626, F-631, F-632, F-633
- `OP-D.H4` Growth dashboard · F-640, F-641
- `OP-D.H5` Campaign manager · F-644
...
- `AF-A.H2` Referral links + conversion · F-802, F-803
- `AF-A.H3` Affiliate onboarding + codes + audit · F-807, F-808, F-809
- `AF-A.H4` Commission + payment · F-804, F-805
...
# RECOVERY LIBRARY (shared)

Each recovery is defined once and reused by every unhappy branch listed in **used by**. Steps are numbered; outcomes use ⤴ rejoin / ⤓ exit.

## Recovery flows (multi-actor)

### `R-KYC-REVIEW` — KYC manual review · F-105
...
- ⤴ verified → continue · ⤓ fail → onboarding paused. **used by:** PF-A.U4

### `R-ELIG-REVIEW` — Paid doctor eligibility review · F-123, F-125
...
- ⤴ approved → consent · ⤓ otherwise → not enrolled (respectful close). **used by:** PF-B.U2, PF-B.U3

### `R-PAY-RETRY` — Payment-failure resolution · F-131
1. Patient retries, switches method, or pays from wallet (idempotent — no double-charge).
2. Persistent failure → finance/CS check.
- ⤴ success → activate · ⤓ final failure → no subscription. **used by:** PF-C.U1

### `R-DUNNING` — Past-due dunning · F-133
1. Show an amber "Payment due" banner with pay-now.
2. Schedule dunning reminders; degrade some features while past due.
- ⤴ pay → active; if grace elapses → `R-REACTIVATE`. **used by:** PF-C.U2

### `R-REACTIVATE` — Expired-mid-treatment reactivation · F-133
...
- ⤴ resubscribe → reactivated. **used by:** PF-C.U3

### `R-REFUND` — Refund / refund-pending · F-147, F-144
1. Finance issues the refund to the wallet.
2. Show the pending entry; a failed refund → `R-CHARGEBACK`.
- ⤴ settled to wallet. **used by:** PF-C.U4

### `R-CHARGEBACK` — Chargeback reconciliation · F-633
...
### `R-RESCHEDULE` — Reschedule a missed/aborted visit · F-422, F-619
...
- ⤴ new visit booked. **used by:** PF-D.U1, PF-E.U1, NF-A.U2, OP-A.U2 (overflow)

### `R-RXFIX` — External Rx re-issue + re-link · F-330
...
- ⤴ prescription active. **used by:** PF-D.U2, DF-A.U2, DF-A.U3

### `R-CLINICAL-ESC` — Urgent side-effect clinical escalation · F-178
...
- ⤴ closed by clinician disposition. **used by:** PF-F.U1

### `R-REPLACE` — Replacement-dose order · F-433, F-504, F-506
...
- ⤓ original visit aborted, replacement dispatched. **used by:** NF-A.U3, R-VIAL-RESCAN, R-RECALL

### `R-VIAL-RESCAN` — Wrong/mismatched vial · F-437
1. If a correct vial is in hand, rescan it and proceed.
2. If none is usable, report it and enter `R-REPLACE`.
- ⤓ proceed with correct vial, or abort + replacement. **used by:** NF-A.U4

### `R-SAFETY-ESC` — Nurse safety escalation · F-497, F-443
1. Fire an immediate must-alert to ops/CS.
2. Ops/CS respond directly and record resolution.
- **used by:** NF-B.U1

### `R-STOCKOUT` — Stock-out / shortage at pickup · F-465, F-504
...
- ⤴ re-dispense or reschedule. **used by:** NF-C.U2, PH-A.U4

### `R-HUB-QUARANTINE` — Hub cold-chain quarantine · F-505, F-506
...
- ⤓ exposed stock quarantined. **used by:** PH-A.U2

### `R-RECALL` — Batch recall → reassign affected patients · F-618
...
### `R-REASSIGN` — Provider reassignment / in-flight care · F: —
...
- ⤴ care continues under the new provider. **used by:** DF-B.U1, DF-B.U3

### `R-CAPACITY` — No nurse available / no capacity · F: —
1. Assign from the board, or push an on-demand assignment.
2. If there's truly no capacity → `R-RESCHEDULE` and notify.
- ⤴ assigned when capacity frees. **used by:** OP-A.U2

### `R-SETTLE-RETRY` — Settlement-run failure · F-629
1. Show per-item results.
2. Retry only the failed items (idempotent rerun — no double-pay).
- ⤴ completes. **used by:** OP-D.U2

## Common handlings (shared UI patterns)

### `R-RESEND-OTP` — Resend a fresh OTP · F-101
1. Show an inline error. 2. Offer "resend" and issue a new OTP. — ⤴ retry. **used by:** PF-A.U1

### `R-COOLDOWN` — Lockout cooldown · F-102
1. Enforce a cooldown window. 2. Offer password recovery as the faster route. — ⤴ retry after cooldown. **used by:** PF-A.U2

### `R-BLOCKED-STATE` — Full-screen blocked/rejected state · F: —
1. Show a factual full-screen state. 2. Route to support/ops. — ⤓ no entry. **used by:** PF-A.U3, AF-A.U2

### `R-INLINE-VALIDATE` — Inline validation + retry · F: —
1. Validate inline (range/type/format) and explain the limit. — ⤴ retry. **used by:** PF-B.U1, MF-A.U1

### `R-EMPTY-STATE` — Designed empty state · F: —
1. Render a friendly empty state with a next action (not an error). **used by:** PF-G.U1, DF-B.U2, NF-C.U3, PH-A.U1

### `R-TRACK-FALLBACK` — Tracking fallback · F-172, F-703
1. Fall back to ETA / last-known position. — ⤴ continue. **used by:** PF-E.U2, NF-C.U1

### `R-GUIDANCE-CTA` — Supportive guidance + sign-up CTA · F-097
1. Show supportive guidance. 2. Offer a sign-up CTA for the full check. **used by:** MF-A.U2

### `R-PREVIEW-RETRY` — Retry / proceed to full check · F-097
1. Offer retry. 2. Or proceed to the full check after sign-up. — ⤴ sign-up. **used by:** MF-A.U3

### `R-COMPLETE-RECORD` — Prompt to complete record · F-312
1. Prompt to complete the missing history/labs. 2. Or proceed with a note. **used by:** DF-A.U1

### `R-GPS-OVERRIDE` — Manual GPS override · F-430
1. Request a supervisor-approved manual override. 2. Log it for audit. — ⤴ continue. **used by:** NF-A.U1

### `R-OTP-RECEIPT` — OTP receipt fallback · F-441
1. Fall back to an OTP receipt to confirm delivery. — ⤴ continue. **used by:** NF-A.U5

### `R-OFFLINE-CAPTURE` — Offline capture + sync · F-430, F-442
1. Capture everything offline. 2. Sync when connectivity returns. **used by:** NF-A.U6

### `R-MANUAL-ENTRY` — Manual barcode entry · F-504
1. Allow manual entry with verification. — ⤴ continue. **used by:** PH-A.U3

### `R-EXPIRE-QUARANTINE` — Quarantine + replace expired stock · F-506
1. Quarantine the expired vial. 2. Replace it. **used by:** PH-A.U5

### `R-ROLE-FIX` — Role/permission correction · F-655
1. Guardrails + audit catch the misconfiguration. 2. Correct the assignment (API stays source of truth). **used by:** OP-A.U1

### `R-GEOCODE-MANUAL` — Manual geocode pin · F-701
1. Allow a manual map-pin. 2. Or retry. — ⤴ continue. **used by:** OP-B.U1

### `R-INCIDENT-TRIAGE` — Incident triage · F-617, F-618
1. Triage on the ops incident feed (the must-alert hub). 2. Drive the matching recovery. **used by:** OP-B.U2

### `R-BULK-RETRY` — Per-row retry · F-644
1. Show per-row results. 2. Retry only the failed rows. **used by:** OP-D.U3

### `R-NEW-LINK` — Issue a fresh referral link · F-802
1. Show a factual state. 2. Issue a new link. **used by:** AF-A.U1

### `R-CS-TICKET` — Generic CS ticket · F-804, F-805
1. Open a standard CS ticket. 2. Apply clawback rules on early churn/refund (window pending). **used by:** AF-A.U3

## Cross-cutting handlings

### `R-OFFLINE-BANNER` — Offline banner · F: —
1. Show a banner. 2. Allow reads; queue writes where safe (nurse captures a full visit offline). **used by:** XF.U1

### `R-REAUTH` — Re-authenticate · F: —
1. Silent token refresh, else prompt re-login. 2. Preserve in-progress form data. — ⤴ resume. **used by:** XF.U2

### `R-NO-ACCESS` — Permission-denied state · F: —
1. Show a full-screen "no access" state (API is source of truth). **used by:** XF.U3

### `R-NOT-FOUND` — Not-found state · F: —
1. Show "no longer available." 2. Send back to the list. **used by:** XF.U4

### `R-RETRY-5XX` — Server-error retry · F: —
1. Silent-retry first. 2. Then a visible state; never lose input. — ⤴ resume. **used by:** XF.U5

### `R-RATE-LIMIT` — Rate-limit wait · F: —
1. Inline/toast with wait guidance. — ⤴ retry. **used by:** XF.U6

### `R-CONFLICT` — Concurrent-edit conflict · F: —
1. Show "this changed since you loaded it." 2. Offer reload/merge. **used by:** XF.U7

---

*Self-contained: the only external reference is the feature id (`F-…`). Flow ids are canonical; recoveries are shared via the library. Companion: `Medica_Flow_Feature_Map.md` maps every flow id to its feature id(s).*
